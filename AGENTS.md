...
KoreanNLPMCPRadar는 한국어 NLP MCP 후보를 수집하고 repository metadata, capability, risk signal을 DuckDB/HTML 리포트로 정리하는 MCP Radar 저장소입니다.
...
│   └── categories/korean_nlp_mcp.yaml   # MCP category 정의
...
- `config/categories/korean_nlp_mcp.yaml`은 MCP 후보 metadata, source contract, activation gate의 기준 파일로 유지한다.
