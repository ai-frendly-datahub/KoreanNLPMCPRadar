# KoreanNLPMCPRadar - 한국어 NLP MCP 레이더

**🌐 Live Report**: https://ai-frendly-datahub.github.io/KoreanNLPMCPRadar/


Korean NLP & Language 섹션의 한국 MCP 서버 목록을 수집하고 리스크/활성도를 추적하는 독립 Radar입니다.

## 프로젝트 목표

- **데이터 수집**: RSS 피드 및 API
- **엔티티 분석**: 도메인별 키워드 매칭
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 한국어 NLP MCP 동향 시각화
...
2. 실행:
   ```bash
   python main.py --category korean_nlp_mcp --recent-days 7
   # 리포트: reports/korean_nlp_mcp_report.html
   ```
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY`는 `korean_nlp_mcp`로 고정합니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `korean_nlp_mcp`인지 확인
...
- Source section: `Korean NLP & Language`
- Primary motion: `intelligence`
- Governance: `medium`
- Data quality priority: `P2`
...
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/KoreanNLPMCPRadar/.
